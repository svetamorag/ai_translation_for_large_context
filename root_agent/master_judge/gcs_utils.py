...
def create_final_gcs_uri(original_gcs_uri: str) -> str:
    ...
    """
    match = re.match(r"gs://([^/]+)/(.+)", original_gcs_uri)
    if not match:
        raise ValueError(f"Invalid GCS URI: {original_gcs_uri}")

    bucket_name, blob_name = match.groups()

    if '/' in blob_name:
        path_prefix = blob_name.rsplit('/', 1)[0]
        original_filename = blob_name.rsplit('/', 1)[1]
        new_blob_name = f"{path_prefix}/final_{original_filename}"
    else:
        new_blob_name = f"final_{blob_name}"
